...
full_data["preference"][full_data["preference"] == -3] <- 1
full_data["preference"][full_data["preference"] == -2] <- 2
full_data["preference"][full_data["preference"] == -1] <- 3
full_data["preference"][full_data["preference"] == 1] <- 4
full_data["preference"][full_data["preference"] == 2] <- 5
full_data["preference"][full_data["preference"] == 3] <- 6
...
fit_SCT_all <- stan(file = here("XPLab_Conformity","Analyses","main","stan_models","SCT.stan"), data=stan_data_all, iter=10000, chains=1, seed = 123, control=list(adapt_delta = 0.99))
fit_herding_all <- stan(file = here("XPLab_Conformity","Analyses","main","stan_models","SCT.stan"), data=stan_data_all, iter=10000, chains=1, control=list(adapt_delta = 0.99))
